...
def get_prices_from_url(url):
    ...
    prices = {}
    offers=soup.findAll('h2')    
    prix=soup.findAll('span',attrs={'class','pricing-table-price'})
    Ul=soup.findAll('ul',attrs={'pricing-table-list'})
    for i in range(len(offers)):
        prices[offers[i].text]={'price':prix[i].text.split()[0],'storage':Ul[i].select('li')[3].text.split()[0],'databases':int(Ul[i].select('li')[4].text.split()[0])}
    return prices
...
def extract_beer_infos(url):
    ...
    content = res.content.decode('utf-8')
    soup=BeautifulSoup(content)
    f=soup.find('div',attrs={'class':'product-subtext'}).find('span').text
...
def extract_beer_list_infos(url):
    ...
    res = requests.get(url)
    content = res.content.decode('utf-8')
    beer_pages=json.loads(content)['items']
    
    # Sequential version (slow):
    beers = []
    for i in range(len(beer_pages)):
        url='https://www.beerwulf.com'+beer_pages[1]['contentReference']
